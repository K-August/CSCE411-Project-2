...
// Main Evaluator
double calc_mHeight_efficient(int n, int k, int m, MatrixXd P, double threshold) {
    // G = [I | P]
    MatrixXd G(k, n);
    ...
    // Initialize HiGHS for this subset
    Highs highs;
    ...
    do {
        ...
        // FIX: Match Python's implicit maximization
        model.lp_.sense_ = ObjSense::kMaximize; 
        
        // FIX: Match SciPy's bounds=(None, None) -> Variables span (-inf, inf)
        model.lp_.col_lower_.assign(k, -kHighsInf);
        model.lp_.col_upper_.assign(k, kHighsInf);

        // FIX: Match Python's A_ub/b_ub constraints -> -1 <= A * x <= 1
        model.lp_.row_lower_.assign(n - m, -1.0);
        model.lp_.row_upper_.assign(n - m, 1.0);

        // Build constraint matrix A = G[:, S_bar]^T in Compressed Column Format
        int num_nz = (n - m) * k;
        ...
        // Pass the structural model to HiGHS
        model.lp_.col_cost_.assign(k, 0.0);
        highs.passModel(model);

        // Solve for each vector in S
        for (int i : S) {
            // Because we set kMaximize, we just grab the exact column (no * -1.0 needed)
            std::vector<double> c(k);
            for(int j = 0; j < k; ++j) {
                c[j] = G(j, i);
            }
            
            // Quickly update the objective without rebuilding the matrix
            ...
            if (status == HighsModelStatus::kOptimal) {
                ...
                return std::numeric_limits<double>::infinity(); // Matches Python res.status == 3
            }
        }
